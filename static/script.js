...
function loadData() {
    d3.json("/data").then(data => {
        ...
        // Initialize cholesterol slider
		d3.select("#cholesterol-slider")
			.on("input", function() {
				// Update cholesterolFilter to range between the slider value and the maximum value
				cholesterolFilter = [+this.value, cholesterolFilter[1]]; 
				console.log("Cholesterol filter updated:", cholesterolFilter); // Debugging log
				d3.select("#cholesterol-value").text(this.value); // Display the current slider value
				const selectedPatientName = document.getElementById('patient-select').value;
				updateFilteredChart(groupedData[selectedPatientName], selectedPatientName);
			});
		...
		d3.select("#bmi-slider")
			.on("input", function() {
				// Update bmiFilter to range between the slider value and the maximum value
				bmiFilter = [+this.value, bmiFilter[1]];
				console.log("BMI filter updated:", bmiFilter); // Debugging log
				d3.select("#bmi-value").text(this.value); // Display the current slider value
				const selectedPatientName = document.getElementById('patient-select').value;
				updateFilteredChart(groupedData[selectedPatientName], selectedPatientName);
			});
